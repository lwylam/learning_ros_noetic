...
int main(int argc, char** argv) {
    ...
    pose_table2_approach.header.frame_id="map"; // "/map"
    ...
    //send vision goal to find coke:
    ROS_INFO("sending a goal: find coke");
    g_goal_done = false;
    ...
    if (g_callback_status!= coordinator::ManipTaskResult::MANIP_SUCCESS)
    {
        ROS_ERROR("failed to find coke quitting");
        return 0;
    }
    ...
    //send command to acquire coke:
    ROS_INFO("sending a goal: grab coke");
    g_goal_done = false;
    ...
    while (!g_goal_done) {
        ...
        if (g_callback_status!= coordinator::ManipTaskResult::MANIP_SUCCESS)
    {
        ROS_ERROR("failed to grab coke; quitting");
        return 0;
    }
    ...
    //send command to set grasped coke on top of existing coke:
    ...
    goal.dropoff_frame.pose.position.z+=0.124; //set height to one coke
                                               // so new coke will stack on prior coke
    goal.object_code= ObjectIdCodes::COKE_CAN_UPRIGHT;
    action_client.sendGoal(goal, &doneCb, &activeCb, &feedbackCb);
    while (!g_goal_done) {
        ros::Duration(0.1).sleep();
